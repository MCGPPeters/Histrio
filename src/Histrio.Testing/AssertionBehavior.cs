...
namespace Histrio.Testing
{
    ...
    /// notified when a message does arrive. This behavior does that using a taskcompletion source. The pattern used is that
    /// one sets the actor hosting this behavior as the ultimate customer of an interaction between actors that needs to be tested.
    /// Than in the await the taskcompletion to get the message that was delivered to the actor hosting this behavior and perform the assertions
